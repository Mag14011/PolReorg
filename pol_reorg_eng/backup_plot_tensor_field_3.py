...
def plot_efield_vector(ax: Axes3D,
    ...
    """
    Ex, Ey, Ez = fields
    # Apply scaling to the field components
    Ex, Ey, Ez = Ex * scale * 10, Ey * scale * 10, Ez * scale * 10  # Added factor of 10

    # Plot main vector with modified parameters
    ax.quiver(center[0], center[1], center[2],
              Ex, Ey, Ez,
              color=color,
              arrow_length_ratio=0.2,
              length=1.0,  # Fixed length parameter
              linewidth=3,
              normalize=True)  # Normalize the vector

    if std is not None and std > 0:
        # Create larger cone of variance
        theta = np.linspace(0, 2*np.pi, 20)
        r = std * scale * 5  # Increased cone size
        x = r * np.cos(theta)
        y = r * np.sin(theta)
