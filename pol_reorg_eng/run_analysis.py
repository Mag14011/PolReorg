...
def process_state_qm_to_md(state_name: str,
    ...
    try:
        ...
        print(f"\nDebug before donor transformation:")
        print(f"donor_transmat path: {donor_transmat}")
        print(f"common_indices length: {len(common_indices)}")
        print(f"common_indices range: {min(common_indices)} to {max(common_indices)}")
        ...
        print(f"\nDebug before acceptor transformation:")
        print(f"acceptor_transmat path: {acceptor_transmat}")
        print(f"common_indices length: {len(common_indices)}")
        print(f"common_indices range: {min(common_indices)} to {max(common_indices)}")
        ...
        # After transforming tensors
        print(f"\nDebug tensor shapes:")
        print(f"donor_diffalpha_trans shape: {donor_diffalpha_trans.shape}")
        print(f"acceptor_diffalpha_trans shape: {acceptor_diffalpha_trans.shape}")
        ...
        print("\nDebug component shapes:")
        print("Donor components lengths:", [len(comp) for comp in donor_components])
        print("Acceptor components lengths:", [len(comp) for comp in acceptor_components])

        # Calculate polarization energies for all frames

        print("\nDebug field shapes:")
        print(f"E_x_donor shape: {len(E_x_donor)}")
        print(f"E_y_donor shape: {len(E_y_donor)}")
        print(f"E_z_donor shape: {len(E_z_donor)}")
        print(f"E_x_acceptor shape: {len(E_x_acceptor)}")
        print(f"E_y_acceptor shape: {len(E_y_acceptor)}")
        print(f"E_z_acceptor shape: {len(E_z_acceptor)}")
        ...
        print(f" polarization_energies_donor = {len(polarization_energies_donor)}")
        print(f" polarization_energies_acceptor = {len(polarization_energies_acceptor)}")

        # Calculate total polarization energies - convert to numpy arrays for element-wise addition
        total_polarization_energies = np.array(polarization_energies_donor) + np.array(polarization_energies_acceptor)
        ...
        print(f" total_polarization_energies = {len(total_polarization_energies)}")
        print(f" coulombic_energy = {len(coulombic_energy)}")
        total_energy = total_polarization_energies + coulombic_energy

        print("\nDebug energy shapes:")
        print(f"polarization_energies_donor shape: {len(polarization_energies_donor)}")
        print(f"polarization_energies_acceptor shape: {len(polarization_energies_acceptor)}")
        print(f"total_polarization_energies shape: {len(total_polarization_energies)}")
        print(f"coulombic_energy shape: {len(coulombic_energy)}")
...
#                                   product_donor_id, product_acceptor_id)
    ...
    # Uncomment this ection to generate an animaiton of the 
    # polarizability ellipsoids and electric field tensors.
    # Generate visualization with frame-by-frame tensors

#   print("\n=== Generating Tensor/Field Visualization ===")
#   animation_output_dir = os.path.join(mp4_output_folder,
#                                     'iterative' if iterative_run else 'single_calc')
#   if not os.path.exists(animation_output_dir):
#       os.makedirs(animation_output_dir)
#       print(f"Created animation output directory: {animation_output_dir}")

#   print("\nDebugging component structures:")
#   print("Reactant donor components type:", type(reactant_results.donor_components))
#   print("Reactant donor components shape/length:", len(reactant_results.donor_components))
#   print("First donor component type:", type(reactant_results.donor_components[0]))
#   print("Reactant donor components:")
#   for i, comp in enumerate(reactant_results.donor_components):
#       print(f"Component {i}: {comp}")

#   print("\nReactant acceptor components type:", type(reactant_results.acceptor_components))
#   print("Reactant acceptor components shape/length:", len(reactant_results.acceptor_components))
#   print("First acceptor component type:", type(reactant_results.acceptor_components[0]))
#   print("Reactant acceptor components:")
#   for i, comp in enumerate(reactant_results.acceptor_components):
#       print(f"Component {i}: {comp}")

#   print("\nPreparing visualization components:")
#   print("1. Constructing tensor arrays...")

#   # For reactant state
#   reactant_donor_frames = len(reactant_fields[-1])
#   reactant_donor_tensors = np.zeros((reactant_donor_frames, 3, 3))
#   # Fill each frame with the same tensor
#   for i in range(reactant_donor_frames):
#       reactant_donor_tensors[i] = np.array([
#           [reactant_results.donor_components[0], reactant_results.donor_components[1], reactant_results.donor_components[2]],
#           [reactant_results.donor_components[1], reactant_results.donor_components[3], reactant_results.donor_components[4]],
#           [reactant_results.donor_components[2], reactant_results.donor_components[4], reactant_results.donor_components[5]]
#       ])
#   print(f"   - Reactant donor tensors: {reactant_donor_tensors.shape}")

#   reactant_acceptor_tensors = np.zeros((reactant_donor_frames, 3, 3))
#   for i in range(reactant_donor_frames):
#       reactant_acceptor_tensors[i] = np.array([
#           [reactant_results.acceptor_components[0], reactant_results.acceptor_components[1], reactant_results.acceptor_components[2]],
#           [reactant_results.acceptor_components[1], reactant_results.acceptor_components[3], reactant_results.acceptor_components[4]],
#           [reactant_results.acceptor_components[2], reactant_results.acceptor_components[4], reactant_results.acceptor_components[5]]
#       ])
#   print(f"   - Reactant acceptor tensors: {reactant_acceptor_tensors.shape}")

#   # For product state
#   product_donor_frames = len(product_fields[-1])
#   product_donor_tensors = np.zeros((product_donor_frames, 3, 3))
#   for i in range(product_donor_frames):
#       product_donor_tensors[i] = np.array([
#           [product_results.donor_components[0], product_results.donor_components[1], product_results.donor_components[2]],
#           [product_results.donor_components[1], product_results.donor_components[3], product_results.donor_components[4]],
#           [product_results.donor_components[2], product_results.donor_components[4], product_results.donor_components[5]]
#       ])
#   print(f"   - Product donor tensors: {product_donor_tensors.shape}")

#   product_acceptor_tensors = np.zeros((product_donor_frames, 3, 3))
#   for i in range(product_donor_frames):
#       product_acceptor_tensors[i] = np.array([
#           [product_results.acceptor_components[0], product_results.acceptor_components[1], product_results.acceptor_components[2]],
#           [product_results.acceptor_components[1], product_results.acceptor_components[3], product_results.acceptor_components[4]],
#           [product_results.acceptor_components[2], product_results.acceptor_components[4], product_results.acceptor_components[5]]
#       ])
#   print(f"   - Product acceptor tensors: {product_acceptor_tensors.shape}")

#   print("\n2. Processing field components...")
#   # Pass frame-by-frame field vectors for animation
#   reactant_donor_fields = reactant_fields[0:3]      # (Ex, Ey, Ez) for each frame
#   reactant_acceptor_fields = reactant_fields[3:6]
#   product_donor_fields = product_fields[0:3]
#   product_acceptor_fields = product_fields[3:6]
#   
#   print("\nFrame counts:")
#   print(f"   - Reactant state: {reactant_frame_count} frames")
#   print(f"   - Product state:  {product_frame_count} frames")
#   
#   print("\n3. Generating animation...")
#   animation_path = ptf.generate_tensor_field_animation(
#       reactant_donor_tensors=reactant_donor_tensors,
#       reactant_acceptor_tensors=reactant_acceptor_tensors,
#       reactant_donor_fields=reactant_donor_fields,      # Frame-by-frame field components
#       reactant_acceptor_fields=reactant_acceptor_fields,
#       product_donor_tensors=product_donor_tensors,
#       product_acceptor_tensors=product_acceptor_tensors,
#       product_donor_fields=product_donor_fields,
#       product_acceptor_fields=product_acceptor_fields,
#       reactant_donor_xyz=paths['reactant_donor_xyz'],
#       reactant_acceptor_xyz=paths['reactant_acceptor_xyz'],
#       product_donor_xyz=paths['product_donor_xyz'],
#       product_acceptor_xyz=paths['product_acceptor_xyz'],
#       output_path=animation_output_dir,
#       first_last_only=False, 
#       frame_interval=100,
#       tensor_scale=5.0,  # Adjust these scaling factors as needed
#       field_scale=4.0     # to make visualization clearer
#   )
#   print(f"\nAnimation saved successfully:")
#   print(f"- Path: {animation_path}")
    ...
    for position, (mean, std) in field_stats.items():
        ...
        print(f"  * Std deviation: {std:.3f} V/Å")
        if use_synthetic_fields:
            target = synthetic_field_params[position]['magnitude']
            print(f"  * Target magnitude: {target:.3f} V/Å")
