...
def plot_efield_vector(ax: Axes3D,
    ...
    Optimized for electric fields in V/Å units.
    """
    Ex, Ey, Ez = fields
    
    # Calculate field magnitude for diagnostic printing
    magnitude = np.sqrt(Ex**2 + Ey**2 + Ez**2)
    print(f"Original E-field magnitude: {magnitude:.3f} V/Å")
    
    # Use a more appropriate base scale for V/Å units
    # We want the vectors to be visible on the scale of our molecular coordinates
    base_scale = 5.0  # Each 0.1 V/Å will show as 0.5 Å in length
    Ex, Ey, Ez = Ex * base_scale * scale, Ey * base_scale * scale, Ez * base_scale * scale
    
    # Print scaled magnitude for verification
    scaled_magnitude = np.sqrt((Ex**2 + Ey**2 + Ez**2))
    print(f"Scaled vector length: {scaled_magnitude:.3f} Å")
    ...
    if std is not None and std > 0:
        # Apply same scaling to std
        scaled_std = std * base_scale * scale
        ...
        if scaled_magnitude > 0:
            direction = np.array([Ex, Ey, Ez]) / scaled_magnitude
            z_axis = direction
