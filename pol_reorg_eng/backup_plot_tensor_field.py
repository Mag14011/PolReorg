...
def plot_ellipsoid(ax: Axes3D, 
    ...
    Args:
        ...
        tensor: 3x3 polarizability tensor
        center: (x,y,z) coordinates of Fe atom
        color: Color for the ellipsoid
        alpha: Transparency (0-1)
        scale: Scaling factor for ellipsoid size
    """
    # Scale the tensor for visualization
    tensor = tensor * scale
    ...
    for i in range(len(x)):
        for j in range(len(x)):
            ...
            transformed = np.dot(eigenvecs, point * np.sqrt(np.abs(eigenvals)))
...
def plot_efield_vector(ax: Axes3D,
    ...
    Args:
        ax: 3D axis to plot on
        fields: (Ex, Ey, Ez) components
        center: Fe atom coordinates as starting point
        color: Color for the vector
        std: Standard deviation for variance cone
        scale: Scaling factor for vector length
    """
    Ex, Ey, Ez = fields
    # Scale the field components
    ...
    if std is not None and std > 0:
        # Create cone of variance
        theta = np.linspace(0, 2*np.pi, 20)
        r = std * np.ones_like(theta) * scale
        x = r * np.cos(theta)
        y = r * np.sin(theta)
...
def generate_tensor_field_animation(
    ...
    center_pos, pos_range = get_position_limits()
    plot_range = pos_range * 1.02  # Add 5% padding
    ...
    def update(frame):
        ...
        for ax, title in [(ax_reactant, "Reactant State"),
            ...
            # Set limits centered around molecular positions
            ax.set_xlim(center_pos[0] - plot_range/2, center_pos[0] + plot_range/2)
            ax.set_ylim(center_pos[1] - plot_range/2, center_pos[1] + plot_range/2)
            ax.set_zlim(center_pos[2] - plot_range/2, center_pos[2] + plot_range/2)
